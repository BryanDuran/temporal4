...
btnEmilinar.addEventListener("click", () => {
    axios.post("http://localhost:4567/usuario", {
        email: document.getElementById("email").value,
        password: document.getElementById("password").value
    })
        .then(function (res) {
            alert("Usuario:" + res.data.status + " id:" + res.data.id + "ha sido eliminado de la lista");
            res.splice(res.data.id);
        })
        .catch(function (error) {
            console.log(error)
        })
...
btnEditar.addEventListener("click", () => {
    axios.post("http://localhost:4567/usuario", {
        ...
        .then(function (res) {
            alert("Usuario:" + res.data.status + " id:" + res.data.id + "ha sido editado");
